...
**Spring Boot Starter，集成 webflux-swagger2**
...
**webflux-swagger2-spring-boot-starter** 是一个面向 使用 webflux-swagger2 的应用 的 Spring Boot Starter，用于将 **webflux-swagger2** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 webflux-swagger2 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 webflux-swagger2 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 webflux-swagger2 Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:webflux-swagger2-spring-boot-starter:1.0.4-SNAPSHOT` |
| 配置前缀 | `webflux.swagger2` |

## 2. 核心能力

| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 webflux-swagger2 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `webflux.swagger2.*` 到 `Swagger2WebFluxProperties` |
| `Swagger2UiWebFluxConfigurer` Bean | ✅ 稳定 | 通过 Swagger2WebFluxAutoConfiguration 自动注册 |
...
| Bean | 条件 | 缺失时行为 |
|---|---|---|
| `Swagger2UiWebFluxConfigurer` | classpath + property | 不创建 |
| `ServiceModelToSwagger2Mapper` | classpath + property | 不创建 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `webflux.swagger2.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `webflux.swagger2.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/webflux-swagger2-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/webflux-swagger2-spring-boot-starter)
